...
float* forward(Transformer* transformer, int token, int pos) {
    ...
    for(unsigned long long l = 0; l < p->n_layers; l++) {
...
#ifdef LLM_EXPT
        float *qq = malloc(dim * sizeof(float));
        memcpy(qq, s->q, dim * sizeof(float));
        float *kk = malloc(kv_dim * sizeof(float));
        memcpy(kk, s->k, kv_dim * sizeof(float));
        rope1(dim, head_size, pos, qq);
        rope2(kv_dim, head_size, pos, kk);
#endif
        ...
        for (h = 0; h < p->n_heads; h++) {
            ...
            // from 0..pos inclusively RS softmax
            softmax(att, pos + 1);
